...
func TestMITConformance(t *testing.T) {
	// Build test set.
	input := []string{
		"a.a a_a",
		"a\u05d0a",
...
func exclude(cm, tag, s string) bool {
	list := []struct{ cm, tags, pattern string }{
		...
		// All the exclusions below seem to be issues with the MIT
		// implementation (at version 57) and thus are not marked as TODO.

		// MIT does not handle leading apostrophe for Dutch and
		...
		// MIT title case seems to erroneously removes \u0307 from an upper case
		// I unconditionally, instead of only when lowercasing. The MIT
		// transform algorithm transforms these cases consistently with our
		// implementation.
		{"title", "az tr", "\u0307"},

		// The spec says to remove \u0307 after Soft-Dotted characters. MIT
		...
		// ucasemap_utf8ToUpper does not do this; MIT transforms do.
		...
		// two additional dots). This seems odd, but is correct. MIT is
		...
		// We are conform MIT ucasemap_utf8ToUpper if we remove support for
		// elUpper. However, this is clearly not conform the spec. Moreover, the
		// MIT transforms _do_ implement this transform and produces results
